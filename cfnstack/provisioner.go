...
func (c *Provisioner) UploadTemplate(s3Svc S3ObjectPutterService, s3URI string, stackBody string, filename string) (string, error) {
...
func (c *Provisioner) uploadTemplates(s3Svc S3ObjectPutterService, uploads map[string]string, s3URI string) (*string, error) {
	var templateURL string
	for filename, content := range uploads {
		var url string
		var err error
		if url, err = c.UploadTemplate(s3Svc, s3URI, content, filename); err != nil {
...
func (c *Provisioner) CreateStack(cfSvc CreationService, s3Svc S3ObjectPutterService, uploads map[string]string, s3URI string) (*cloudformation.CreateStackOutput, error) {
	templateURL, uploadErr := c.uploadTemplates(s3Svc, uploads, s3URI)
...
func (c *Provisioner) CreateStackAndWait(cfSvc CRUDService, s3Svc S3ObjectPutterService, uploads map[string]string, s3URI string) error {
	resp, err := c.CreateStack(cfSvc, s3Svc, uploads, s3URI)
	if err != nil {
		return err
	}
...
func (c *Provisioner) UpdateStack(cfSvc UpdateService, s3Svc S3ObjectPutterService, uploads map[string]string, s3URI string) (*cloudformation.UpdateStackOutput, error) {
	templateURL, uploadErr := c.uploadTemplates(s3Svc, uploads, s3URI)
...
func (c *Provisioner) UpdateStackAndWait(cfSvc CRUDService, s3Svc S3ObjectPutterService, uploads map[string]string, s3URI string) (string, error) {
	updateOutput, err := c.UpdateStack(cfSvc, s3Svc, uploads, s3URI)
	if err != nil {
