...
func runCmdUpdate(cmd *cobra.Command, args []string) error {
	...
	if err := confCluster.ValidateUserData(stackTemplateOptions); err != nil {
		return err
	}

	stackConfig, err := confCluster.RenderStackTemplate(stackTemplateOptions, updateOpts.prettyPrint, updateOpts.s3URI)
	if err != nil {
		return fmt.Errorf("Failed to render stack template: %v", err)
	}

	cluster := cluster.New(stackConfig, updateOpts.awsDebug)
